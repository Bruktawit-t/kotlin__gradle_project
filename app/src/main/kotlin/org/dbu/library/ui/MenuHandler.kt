...
fun handleMenuAction(
    ...
    return when (choice) {
        ...
        "7" -> { viewAllPatrons(repository); true }
        "0" -> false
        else -> {
...
fun addBook(service: LibraryService) {
    ...
    val book = Book(isbn, title, author, year, true)
...
fun viewAllPatrons(repository: LibraryRepository) {
    val patrons = repository.getAllPatrons()
    if (patrons.isEmpty()) {
        println("No patrons registered")
    } else {
        patrons.forEach { patron ->
            println("ID: ${patron.id}, Name: ${patron.name}, Email: ${patron.email}, Phone: ${patron.phone}")
            println("  Borrowed Books: ${patron.borrowedBooks.joinToString(", ")}")
        }
    }
}